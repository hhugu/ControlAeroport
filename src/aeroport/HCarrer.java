...
public class HCarrer extends Carrer {
	...
	public HCarrer(String idWay, boolean nomesUnAvio, int cmWayWidth, int cmLong, int cmPosIniX, int cmPosIniY) {
	...
	public CrossRoad intersectedCrossRoad(int cmPosition) {
		...
		while (itr.hasNext()) {
			...
			if (this.insideThisCrossRoad(cmPosX, cr)) return cr;
		}
	...
	public int getCmPosX(int cmPosition, Avio.Direction direction) {
		...
		if (cmPosX < this.cmIniX || cmPosX > this.cmFinX) return -1; //Fuera de la via

		return cmPosX;
	}

	public int getCmPosY(int cmPosition, Avio.Direction direction) {
		if (direction == Avio.Direction.FORWARD) return this.cmFinY - (this.cmWidth / 4);
		else return this.cmIniY + (this.cmWidth / 4);
	}
    ...
    public void paint(Graphics g, float factorX, float factorY, int offsetX, int offsetY) {
        ...
        // Road
        g2d = (Graphics2D) g;
        GradientPaint gp5 = new GradientPaint(0, yIni + 2, Color.decode("0x404040"), 0, yIni + wayWidth / 2.9F, Color.decode("0x606060"), true);
        g2d.setPaint(gp5);
